  ...
  Value *VisitUnaryAddrOf(const UnaryOperator *E) {
    ...
    if (TI.areAllPointersCapabilities()) {
      ...
      if (AddrTy->getPointerAddressSpace() != CapAS) {
        if (cast<FunctionType>(E->getSubExpr()->getType().getDesugaredType(CGF.getContext()))->getCallConv() != CC_CHERICCallback) {
...
Value *ScalarExprEmitter::VisitCastExpr(CastExpr *CE) {
  ...
  case CK_PointerToCHERICapability: {
    ...
    if (TI.SupportsCapabilities()) {
      if (SrcPointeeTy->isFunctionType() && DstPointeeTy->isFunctionType() &&
          cast<FunctionType>(SrcPointeeTy.getDesugaredType(CGF.getContext()))->getCallConv() != CC_CHERICCallback) {
  ...
  case CK_FunctionToPointerDecay: {
    ...
    if (TI.areAllPointersCapabilities()) {
      unsigned CapAS = CGF.CGM.getTargetCodeGenInfo().getCHERICapabilityAS();
      if (AddrTy->getPointerAddressSpace() != CapAS) {
        if (cast<FunctionType>(E->getType().getDesugaredType(CGF.getContext()))->getCallConv() != CC_CHERICCallback) {
          Addr = CodeGenFunction::FunctionAddressToCapability(CGF, Addr);
        } else {
          auto *VTy = cast<llvm::PointerType>(Addr->getType());
          unsigned CapAS = CGF.CGM.getTargetCodeGenInfo().getCHERICapabilityAS();
