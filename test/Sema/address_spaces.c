         ...
         _AS1 float b) // expected-error {{parameter may not be qualified with an address space}}
{
  ...
  __attribute__((address_space(0x1FFFFF))) int *_boundsB;
  __attribute__((address_space(0x200000))) int *_boundsC; // expected-error {{address space is larger than the maximum supported}}
